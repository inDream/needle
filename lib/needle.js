    ...
    var request = protocol.request(request_opts, function(resp) {
      ...
      if(config.output && config.real_path){
        ...
        if(mime.type){          
          ...
          if(mime_ext && filename.indexOf('.' + mime_ext) < 0 && _mime.lookup(filename) != mime.type )filename += '.' + mime_ext;
          else if(!_mime.lookup(filename)){filename = filename + '.' + mime.type.match(/\/(.*)/)[1];}
        }
      ...
      // decoding non UTF-8 bodies to UTF-8, using the iconv-lite library.
        ...
        && mime.charset && !mime.charset.match(/utf-?8$/i)) {
          pipeline.push(decoder(config.charset || mime.charset));
      }
      ...
      if (config.output && resp.statusCode == 200) {
        resp.pipe(fs.createWriteStream(config.real_path ? real_path : config.output));
      }
